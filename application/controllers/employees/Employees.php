...
class Employees extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = __session('employees');
	...
	public function profile() {
		...
		if (_isNaturalNumber( $id )) {
			$this->vars['query'] = $this->employees->profile($id);
			$this->vars['title'] = 'Profil ' . __session('employees');
			$this->vars['photo'] = base_url('media_library/employees/');
	...
	private function dataset() {
		return [
			...
			'nik' => $this->input->post('nik', true) ? $this->input->post('nik', true) : NULL,
	...
	public function employee_reports() {
		if ($this->input->is_ajax_request()) {
			$query = $this->m_employees->employees_reports();
			$this->output
				->set_content_type('application/json', 'utf-8')
